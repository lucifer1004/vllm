...
def _flashinfer_autotune_active() -> bool:
    try:
...
# FlashInfer IPC launch-config resolution is collective over the TP group.
# The sparse-MLA autotune warmup profiles on the leader rank only, so a
# first-time resolution there would interleave different collectives per rank
# and wedge the group (observed as a 600 s NCCL watchdog timeout on the
# two-element agreement all-reduce). The warmup sets this flag on EVERY rank,
# keeping the opt-out symmetric.
_FLASHINFER_IPC_WARMUP_SUPPRESS = False


@contextmanager
def suppress_flashinfer_ipc_allreduce():
    """Keep the flashinfer-ipc PCIe backend out of a warmup scope, all ranks."""
    global _FLASHINFER_IPC_WARMUP_SUPPRESS
    previous = _FLASHINFER_IPC_WARMUP_SUPPRESS
    _FLASHINFER_IPC_WARMUP_SUPPRESS = True
    try:
        yield
    finally:
        _FLASHINFER_IPC_WARMUP_SUPPRESS = previous
...
class CustomAllreduce:
    ...
    def __init__(
        ...
        self._pcie_hidden_size: int | None = None
        ...
        if integrated_pcie_requested:
            if not same_node or not current_platform.is_cuda():
                logger.warning(
                    "Custom allreduce is disabled because the integrated "
                    "PCIe oneshot backend requires a single-node CUDA topology."
                )
                return
            logger.debug(
        ...
        elif same_node and not fully_connected:
        ...
        if use_pcie_oneshot:
            ...
            self._pcie_hidden_size = model_hidden_size
    ...
    def prepare_pcie_allreduce_shapes(self, token_counts: Iterable[int]) -> None:
        """Resolve FlashInfer IPC launch configs for CUDA-graph capture shapes.

        FlashInfer's IPC workspace resolves a shape's launch configuration
        lazily through a collective agreement, which cannot run inside a CUDA
        graph capture. Resolving every capture shape up front moves those
        collectives to this caller-chosen point.

        Collective: every rank must pass the same token counts in the same
        order. Call it with the active capture channel already bound (inside
        ``capture()``), so the resolution lands on the channel the graph will
        use. Backends without lazy collective resolution (b12x) are a no-op.
        """
        runtime = self._pcie_runtime
        if runtime is None or self._pcie_backend_name != "flashinfer-ipc":
            return
        prepare = getattr(runtime, "prepare_active_shapes", None)
        if prepare is None or self._pcie_hidden_size is None:
            return
        shapes = sorted(
            {(int(count), self._pcie_hidden_size) for count in token_counts}
        )
        if shapes:
            prepare(shapes)
    ...
    def should_custom_ar(self, inp: torch.Tensor):
        if self.disabled or self.world_size > 8:
            return False
        if self._pcie_runtime is not None:
            # The sparse-MLA autotune warmup profiles on the leader rank only;
            # a first-time collective launch-config resolution on a follower
            # would deadlock against the leader's profiling loop. The warmup
            # suppresses this backend on every rank instead (see
            # suppress_flashinfer_ipc_allreduce).
            if (
                self._pcie_backend_name == "flashinfer-ipc"
                and _FLASHINFER_IPC_WARMUP_SUPPRESS
            ):
                return False
