...
class FlashInferPcieIpcAllReducePool:
    ...
    def prepare_active_shapes(
        self,
        shapes: Sequence[tuple[int, int]],
        *,
        dtype: torch.dtype = torch.bfloat16,
    ) -> None:
        """Resolve launch configs on the active (capture) channel's workspace.

        FlashInfer resolves a shape's launch configuration through a
        collective agreement that cannot run during CUDA graph capture. Every
        rank must pass the same shapes in the same order; the resolution lands
        on the workspace of the channel this pool is currently capturing with
        (or the eager channel outside a capture context).
        """
        channel_id = self._active_channel_id or self._EAGER_CHANNEL_ID
        self._workspace(channel_id).prepare(shapes, dtype=dtype)
