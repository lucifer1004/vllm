...
class NixlBaseConnectorWorker:
    ...
    def __init__(
        ...
        self._uses_packed_kv_cache = any(
            tensor.block_stride > 0 for tensor in self.kv_cache_config.kv_cache_tensors
        )
    ...
    def _describe_packed_kv_cache(self, block_stride: int) -> list[NixlPackedKVRegion]:
    ...
    def _build_packed_remote(self, nixl_agent_meta: NixlAgentMetadata) -> np.ndarray:
        ...
        remote_by_layer = {
            (region.group_id, region.layer_name): region for region in remote_regions
        }
        ...
        for local_region in self._packed_kv_cache_regions:
            ...
            parts.append(
                self._stack_descs(addrs, local_region.length, nixl_agent_meta.device_id)
            )
        return np.concatenate(parts)
    ...
    def _prepare_packed_pp_pull_handles(
        ...
        for remote_region in remote_regions:
            layout_key = (remote_region.group_id, remote_region.layer_name)
            local_region = local_by_layer.get(layout_key)
        ...
        transfer_key = (remote_pp_rank, remote_tp_rank)
        ...
        self._packed_pp_pull_src_handles[nixl_agent_meta.engine_id][transfer_key] = (
            self.nixl_wrapper.prep_xfer_dlist("NIXL_INIT_AGENT", local_descs)
        )
        self._packed_pp_pull_dst_handles[nixl_agent_meta.engine_id][transfer_key] = (
            self.nixl_wrapper.prep_xfer_dlist(remote_agent_name, remote_descs)
        )

        num_groups = len(self.kv_cache_config.kv_cache_groups)
        self._packed_pp_pull_region_indices[nixl_agent_meta.engine_id][transfer_key] = [
            [
                region_idx
