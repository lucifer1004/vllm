...
def _get_rope_parameters(
    rope_scaling: dict[str, Any] | None, rope_theta: float
) -> dict[str, Any]:
    rope_parameters = dict(rope_scaling or {})
    # Hunyuan's image branch uses a custom rope config for diffusion, but the
    # text path here uses the default rotary embedding.
...
class HunYuanAttention(nn.Module):
    def __init__(
        ...
        self.rotary_emb = get_rope(
            ...
            rope_parameters=_get_rope_parameters(rope_scaling, rope_theta),
            is_neox_style=True,
        )
        self.attn = Attention(
            self.num_heads,
...
class HunyuanImage3Model(nn.Module):
    ...
    def forward_block(
        self, hidden_states, attention_mask, custom_pos_emb, residual=None
    ):
        prev_kv_states = None
...
class HunyuanImage3ForCausalMM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def forward_block(
        ...
        model_output = self.model.forward_block(
