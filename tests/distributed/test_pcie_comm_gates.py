...
class _FakeDriver:
    ...
    def __init__(self):
        self.calls = []
        self.CUstreamWaitValue_flags = SimpleNamespace(CU_STREAM_WAIT_VALUE_GEQ=1)
...
def test_pcie_barrier_slots_and_sequence(barrier_env):
    drv = barrier_env
    ...
    assert {a for _, a, _ in writes} == {
        h.signal_pad_ptrs[p] + base + 4 * h.rank for p in range(4)
    }
    # every wait polls this rank's own pad, one slot per sender
    assert {a for _, a, _ in waits} == {
        h.signal_pad_ptrs[h.rank] + base + 4 * p for p in range(4)
    }
    ...
    # second barrier on the same channel bumps the sequence
    drv.calls.clear()
    spb._pcie_safe_barrier(h, channel=channel)
    assert all(v == 2 for _, _, v in drv.calls)

    # other channels keep independent sequences
    drv.calls.clear()
    spb._pcie_safe_barrier(h, channel=0)
    assert all(v == 1 for _, _, v in drv.calls)
